...
keyboard_main.add_button(label='показать избранное', color=VkKeyboardColor.POSITIVE)
    ...
    def send_user_info(self, user_id):  
        ...
        while True: 
            ...
            if main.check_users(str(self.candidate['owner_id'])):
                self.candidate = self.vk_api.search_couple(user, self.offset)
                main.add_new_user(self.candidate)
                msg = (f"Как тебе? {self.candidate['first_name']} "
                       f"{self.candidate['last_name']}. Вот ссылка на профиль - "
                       f"{self.candidate['user_link']}. А вот фотографии: ")
