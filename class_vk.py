...
class VK:
    ...
    def user_info_self(self, user_id):
        response = self.vk_api.method('users.get', {'user_ids': user_id})
    ...
    def get_photo(self, owner_id, weight=3):
        try:
            attachments = []
    ...
    def search_couple(self, user_info):
        try:
            ...
            for user in users['items']:  # итерируем для возвращения результата поиска
                ...
                # result_bot.send_msg(user_id_source, result_message)
                # return 'готово'
